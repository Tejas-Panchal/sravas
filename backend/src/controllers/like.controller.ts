...
const togglePostLike = asyncHandler(async (req, res) => {
  ...
  const like = await Like.create({ post: postId, likedBy: req.user._id });
  if (!like) throw new ApiError(500, "something went wrong while liking post");

  return res
    .status(201)
    .json(new ApiResponse(201, { isLiked: true }, "post liked"));
});
